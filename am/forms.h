...
public:
	Admin()
	{
		...
		std::cin.get();
		std::cout << "ընտրել առարկան : ";
		std::getline(std::cin, name);
		out.open(name + ".txt", std::ofstream::app);
		std::cout << "նշել հարցերի քանակը: ";
...
private:
	void set_question()
	{
		...
		int is_true;
		std::cin.get();
		std::cout << "մուտքագրել հարցը : ";
		std::getline(std::cin, question);
		for (int i = 0; i < 4; ++i)
		{
			std::cout << "մւտքագրել պատասխանը: ";
			std::getline(std::cin, answer[i]);
		}
		std::cout << "մւտքագրել ճիշտ պատասխանը: ";
		std::cin >> is_true;
...
public:
	Student()
	{
		...
		std::cout << "ձեր հավաքած բալերի քանակը ֊» " << bal << " " << std::endl;
